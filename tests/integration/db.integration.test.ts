...
describe('openSidepadDb', () => {
  it('creates a fresh DB at the given path with all migrations applied', () => {
    ...
    // Migration 025 — session collaboration mode
    const cols = db.prepare('PRAGMA table_info(sessions)').all().map((c: any) => c.name);
    expect(cols).toContain('collab_mode');
    db.close();
  });
...
describe('_loadMigrationsFromFs', () => {
  it('exposes all SQL migrations in version order with the expected first-table content', () => {
    ...
    expect(ms.map((m) => m.version)).toEqual([1, 2, 3, 4, 5, 6, 17, 18, 19, 20, 21, 22, 23, 24, 25]);
