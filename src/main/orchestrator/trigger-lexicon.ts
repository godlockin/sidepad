...
const roundtableVerbsZh = /圆桌|一起讨论|大家一起|共同讨论|各抒己见|讨论一下/;
const roundtableVerbsEn = /\broundtable\b|\bround table\b|\bpanel discussion\b|\bdiscuss together\b|\blet's discuss\b|\ball voices\b/i;
...
export function scanRoundtable(text: string): boolean {
  return roundtableVerbsZh.test(text) || roundtableVerbsEn.test(text);
}
